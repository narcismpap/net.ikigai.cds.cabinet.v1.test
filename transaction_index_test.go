...
func IndexWithoutPayload(i *pb.Index) *pb.Index{
	i2 := i
	i2.Properties = nil
	return i2
}

func IndexWithValuePayload(i *pb.Index, value string, payload []byte) *pb.Index{
	i2 := i
	i2.Properties = payload
	i2.Value = value
	return i2
...
func TestTransactionIndexBatch(t *testing.T) {
	...
	// drop every even indexes
	for i := range indexes{
		if i % 2 == 0 {
			trx[pos] = pb.TransactionAction{
				ActionId: pos, Action: &pb.TransactionAction_IndexDelete{IndexDelete: IndexWithoutPayload(indexes[i])},
			}
		}
	}
